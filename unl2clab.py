...
# {"EVE-NG のテンプレート名": "Containerlab の kind 名"}
MAPPING = {
    "xrv": "cisco_xrv",
    "xrv9k": "cisco_xrv9k",
    ...
    "vios": "linux",
    "vjunosevoefi": "juniper_vjunosevolved",
    "vjunosevo": "juniper_vjunosevolved",
    "vjunosevo23": "juniper_vjunosevolved",
}

REGISTRY = "vrnetlab"

def __parse_unl(file_path):
    tree = ET.parse(file_path)
    root = tree.getroot()
    return root


def convert_unl_to_clab(file_path):
    root = __parse_unl(file_path)
    # lab の名前
    name = "".join([ c if c.isalnum() else "-" for c in root.get("name") ]).lower()
    # clab config が置かれるディレクトリの名前
    # .gitignore で除外している
    ...
    raw_nodes = {}
    links = {}
    # {"EVE の image名": {"image": "<registry>/<node_kind>","version": "container の version tag"} }
    # topo から node 情報と link 情報を取得
    ...
    # startup config を取得
    if root.find("objects") is not None:
    ...
    for x in raw_nodes:
        ...
        if 'config' in raw_nodes[x]:
            ...
            nodes[node_name] = {'kind': node_kind,'image': node_image, 'startup-config': cfg_rel_path}
        else:
            nodes[node_name] = {'kind': node_kind, 'image': node_image}
